import { syncMyCartActivity, type CartActivityPayload } from "@/services/journey";
...
function toPayload(state: CartStoreState, isActiveOverride?: boolean): CartActivityPayload | null {
  if (!state.cartId) {
    return null;
  }
...
export function startCartActivitySync(): void {
  ...
  if (initialPayload && initialPayload.is_active) {
  ...
  unsubscribeCartStore = useCartStore.subscribe((state, prevState) => {
    ...
    if (serializeRelevantState(state) === serializeRelevantState(prevState)) {
      return;
    }
    ...
    if (nextPayload && nextPayload.is_active) {
