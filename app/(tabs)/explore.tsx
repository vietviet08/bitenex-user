  ...
  return (
    <View className="px-lg pt-xl">
      <View className="flex-row items-center mb-md">
        ...
        <Text className="text-sm font-medium text-text-tertiary">Gợi ý tìm kiếm thông minh</Text>
      </View>
      <View className="flex-row flex-wrap" style={{ gap: 8 }}>
        {QUICK_SUGGESTIONS.map((suggestion) => (
          ...
          >
            <Text className="text-sm font-medium text-primary-700">{suggestion}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </View>
...
function MatchScoreBar({ score }: { score: number }) {
  ...
  const color =
    pct >= 80 ? colors.primary[500] : pct >= 60 ? colors.warning : colors.neutral[300];
    ...
    >
      ...
      <View className="flex-1">
        <View className="flex-row items-center justify-between">
          <Text className="text-base font-semibold text-text-primary flex-1 mr-sm" numberOfLines={1}>
            {item.name}
          </Text>
          {/* Type badge */}
          <View
          ...
          >
            <Text className={`text-xs font-medium ${isFood ? 'text-secondary-700' : 'text-primary-700'}`}>
        ...
        {/* Description */}
        {item.description && (
          <Text className="text-sm text-text-secondary mt-xs" numberOfLines={2}>
            {item.description}
          </Text>
        )}

        {/* Meta info */}
        <View className="flex-row items-center mt-xs flex-wrap" style={{ gap: 8 }}>
          {isFood && item.merchant_name && (
            <Text className="text-xs text-text-tertiary">🏪 {item.merchant_name}</Text>
          )}
          {isFood && item.price && (
          ...
          {!isFood && item.average_rating && item.average_rating > 0 && (
            <Text className="text-xs text-text-tertiary">⭐ {item.average_rating.toFixed(1)}</Text>
          )}
          {!isFood && item.delivery_fee !== null && (
            <Text className="text-xs text-text-tertiary">
              🛵 {item.delivery_fee === 0 ? 'Miễn phí' : `${item.delivery_fee.toLocaleString('vi-VN')} ₫`}
            </Text>
          )}
        </View>
...
function ResultSkeleton() {
  ...
  return (
    <Animated.View style={{ opacity }}>
      {[1, 2, 3, 4].map((i) => (
        <View key={i} className="flex-row items-start px-lg py-md border-b border-border-light">
...
export default function ExploreScreen() {
  const { results, isLoading, error, usedFallback, search, clearResults, searchType, setSearchType } =
  ...
  const [placeholderIndex] = useState(() => Math.floor(Math.random() * NATURAL_PLACEHOLDERS.length));
    ...
    <SafeAreaView className="flex-1 bg-background-primary" edges={['top']}>
      ...
      <View className="px-lg pt-md pb-sm border-b border-border-light">
        ...
        <View className="flex-row items-center mb-sm">
          <Text className="text-2xl font-bold text-text-primary flex-1">Khám phá</Text>
          {!isSearching && (
            <View className="flex-row items-center bg-primary-50 px-sm py-xs rounded-full">
              <Text className="text-xs mr-xs">✨</Text>
              <Text className="text-xs font-medium text-primary-600">AI Search</Text>
            </View>
          )}
        </View>
      ...
      {!isSearching ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={() => (
            <>
              <QuickSuggestions onSelect={handleSuggestionPress} />

              {/* Popular Cuisines */}
              <View className="px-lg pt-xl pb-lg">
                <Text className="text-xl font-semibold text-text-primary mb-md">Ẩm thực phổ biến</Text>
                <View className="flex-row flex-wrap" style={{ gap: 12 }}>
                  {[
                    ...
                    <TouchableOpacity
                      key={cuisine.name}
                      onPress={() => handleSuggestionPress(`Đồ ăn ${cuisine.name}`)}
                      className="w-[30%] bg-neutral-50 border border-border-light p-lg rounded-xl items-center"
                    >
                      <Text className="text-[32px] mb-xs">{cuisine.icon}</Text>
        ...
        <>
          ...
          {!isLoading && results.length > 0 && (
            <View className="flex-row items-center justify-between px-lg py-sm">
              ...
              <Text className="text-xs text-text-tertiary">{results.length} kết quả</Text>
            </View>
          )}
          ...
          {error && !isLoading && (
            <View className="items-center py-xl px-lg">
              ...
              <Text className="text-base font-semibold text-text-primary mb-xs">Có lỗi xảy ra</Text>
              <Text className="text-sm text-text-tertiary text-center">{error}</Text>
            </View>
          )}
